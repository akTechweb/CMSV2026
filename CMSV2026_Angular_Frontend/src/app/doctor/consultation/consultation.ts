...
export class DoctorConsultation implements OnInit {
  ...
  ngOnInit(): void {
    this.route.paramMap.subscribe((params) => {
      ...
      // Reset all state left over from any previous appointment/consultation
      // before loading the new one — otherwise a stale error/summary from
      // the last appointment stays on screen since Angular reuses this
      // component instance when only the route param changes.
      this.model = null;
      this.summary = null;
      this.error = '';
      this.loading = true;
