        ...
        public List<Bill> GetAllBills()
        {
            ...
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        Bill bill = MapBillHeader(reader);

                        // Pharmacy bills are header rows in the same Bills table but are
                        // never linked to an appointment. Exclude them here so the
                        // receptionist's bill list only shows reception/consultation bills.
        ...
        public Bill GetBillById(int billId)
        {
            ...
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    ...
                    // A pharmacy bill can share the Bills table but is never linked to an
                    // appointment. Treat it as not found here so reception's "view" and
                    // "print PDF" actions can't pull up a pharmacy bill either.
                    if (bill.AppointmentId == null) return null;
